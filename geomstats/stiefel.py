...
class Stiefel(EmbeddedManifold):
    ...
    def random_uniform(self, n_samples=1):
        ...
        return gs.matmul(Z, gs.linalg.inv(matrix_f(gs.matmul(gs.transpose(Z, axes=(0, 2, 1)), Z), gs.sqrt)))
...
class StiefelMetric(RiemannianMetric):
    ...
    def exp(self, tangent_vec, base_point):
        ...
        MNe = matrix_f(block, gs.exp)
        
        return gs.matmul(gs.concatenate([base_point, Q], axis=2), MNe[:, :, 0:p])
    ...
    def log(self, point, base_point, max_iter=100, tol=1e-6):
        ...
        V = gs.zeros((W.shape[0], max(W.shape[1], W.shape[2]), max(W.shape[1], W.shape[2])))
        ...
        V[:, :, p:2*p] = gs.matmul(V[:, :, p:2*p], gs.matmul(R, gs.transpose(D, axes=(0, 2, 1))))
        V = gs.concatenate([gs.concatenate([M, N], axis=1), V[:, :, p:2*p]], axis=2)
        
        for k in range(max_iter):
            
            LV = matrix_f(V, gs.log)
            ...
            Phi = matrix_f(-C, gs.exp)
            V[:, :, p:2*p] = gs.matmul(V[:, :, p:2*p], Phi)
            
        return gs.matmul(base_point, LV[:, 0:p, 0:p]) + gs.matmul(Q, LV[:, p:2*p, 0:p])
    ...
    def retraction(self, tangent_vec, base_point):
        ...
        R = gs.zeros((base_point.shape[0], base_point.shape[2], base_point.shape[2]))
        ...
        for i, _ in enumerate(R):
            result[i] = gs.matmul(Q, gs.diag(gs.sign(gs.sign(gs.diagonal(R[i])) + 0.5)))
