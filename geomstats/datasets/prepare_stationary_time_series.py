...
def burg_regularized_for_one_time_series(
        data,
        model_order=4,
        gamma='default'):
    ...
    for i in range(1, model_order+1):
        ...
        power_and_reflection_coefficients[i] = \
            - (2 / (n_pulses - i) * sum1 + 2 * sum2) / (1 / (n_pulses - i) * sum3 + 2 * sum4)
        a[i, i] = power_and_reflection_coefficients[i]
        if i <= model_order - 1:
            for k in range(1, i):
                a[k, i] = a[k, i - 1] + power_and_reflection_coefficients[i] * gs.conj(a[i-k, i-1])
            for k in range(i, n_pulses):
                f[i, k] = f[i - 1, k] + power_and_reflection_coefficients[i] * b[i-1, k-1]
                b[i, k] = b[i - 1, k - 1] + gs.conj(power_and_reflection_coefficients[i]) * f[i-1, k]
...
def burg_regularized(data, model_order=4, gamma='default'):
    ...
    model_order = int(min(model_order, n_pulses - 1))
    power_and_reflection_coefficients = gs.zeros([n_samples, model_order+1], dtype=complex)
    for i_sample in range(n_samples):
        power_and_reflection_coefficients[i_sample, :] = burg_regularized_for_one_time_series(
            data[i_sample, :], model_order, gamma)
    return power_and_reflection_coefficients
