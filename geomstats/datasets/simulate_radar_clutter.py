...
def gaussian_correlation_vector(t, mean=0, variance=1):
    ...
    """
    correlation_vector = np.exp(1j * 2 * np.pi * mean * t) * np.exp(
        -2 * np.pi**2 * variance * t**2
    )
...
def triangular_correlation_vector(t, mean=0, variance=1):
    ...
    correlation_vector = (
        np.exp(1j * 2 * np.pi * mean * t) * (1 / a_sinc) * np.sinc(t / a_sinc) ** 2
    )
    return correlation_vector
...
def exp_abs_correlation_vector(t, mean=0, variance=1):
    ...
    correlation_vector = (
        np.exp(1j * 2 * np.pi * mean * t)
        * (2 * a_abs)
        / (a_abs**2 + 4 * np.pi**2 * t**2)
    )
    return correlation_vector
...
def geometric_correlation_vector(t, mean=0, variance=1 / 2):
    ...
    if variance <= 0 or variance >= 1:
        raise ValueError("The parameter should be positive and lower than one.")
    # correlation_vector = np.exp(
    #     1j * 2 * np.pi * mean * t) * np.exp(- a_abs * (np.abs(t)))
    correlation_vector = np.exp(1j * 2 * np.pi * mean * t) * variance ** (np.abs(t))
    return correlation_vector
...
def simulate_clutter(
    ...
    temporal_correlation_vector = temporal_correlation_function(
        t, mean=spectrum_mean, variance=spectrum_variance
    )
    ...
    square_root_temporal_correlation_matrix = (
        eigenvectors @ np.diag(square_root_eigenvalues) @ eigenvectors.conj().T
    )
    ...
    spatial_correlation_vector = spatial_correlation_function(
        t, mean=0, variance=spatial_correlation_coefficient
    )
    spatial_correlation_matrix = toeplitz(spatial_correlation_vector)
    eigenvalues, eigenvectors = eigh(spatial_correlation_matrix)
    square_root_eigenvalues = eigenvalues**0.5
    square_root_spatial_correlation_matrix = (
        eigenvectors @ np.diag(square_root_eigenvalues) @ eigenvectors.conj().T
    )

    """Simulate gaussian random matrix."""
    gaussian_matrix = (
        np.random.randn(n_cells, n_pulses) + 1j * np.random.randn(n_cells, n_pulses)
    ) / np.sqrt(2)
    ...
    dataset = (
        square_root_spatial_correlation_matrix
        @ gaussian_matrix
        @ square_root_temporal_correlation_matrix.T
    )
    ...
    if thermal_noise_power > 0:
        dataset += (
            np.sqrt(thermal_noise_power)
            * (
                np.random.randn(n_cells, n_pulses)
                + 1j * np.random.randn(n_cells, n_pulses)
            )
            / np.sqrt(2)
        )
