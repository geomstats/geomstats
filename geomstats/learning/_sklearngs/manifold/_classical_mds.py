...
class ClassicalMDS(BaseEstimator):
    ...
    def fit_transform(self, X, y=None):
        ...
        B -= gs.mean(B, axis=0)
        B -= gs.mean(B, axis=1, keepdims=True)
        B *= -0.5

        # Eigendecomposition
        # TODO (L): backend issue
        # w, U = linalg.eigh(B)
        w, U = gs.linalg.eigh(B)
        ...
        w = gs.flip(w, axis=0)[: self.n_components]
        U = gs.flip(U, axis=1)[:, : self.n_components]
