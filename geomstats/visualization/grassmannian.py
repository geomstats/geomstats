...
def projection_to_three_d(projections, points):
    """Take Grassmannian projection matrix -> plotable 3D point.
...
def two_d_to_projection(vector):
    """Use 2D point on manifold and gets its corresponding projection.
...
@gs.autodiff.custom_gradient(_squared_dist_grad_point_a,
                             _squared_dist_grad_point_b)
...
class Grassmannian:
    ...
    def random_uniform(self, n_samples=1):
        ...
        projector = Matrices.mul(
            points, GeneralLinear.inverse(full_rank),
    ...
    def plot(self, ticks):
        """Graph a Grassmannian manifold using the projection matricies(k=1 and n=2,3).

        INPUT: Grassmannian manifold and True/False
        for ticks to determine if you want labels
        ...
        if n == 2:
            ...
            for i in range(np.shape(array_of_2d_vectors)[0]):
                plt.plot(array_of_2d_vectors[i][0],
                         array_of_2d_vectors[i][1], "r.")
            if ticks is True:
                pi = np.pi
                thetas = [
                    pi / 2,
        ...
        if n == 3:
            ...
            for i in range(points):
                vector = np.random.rand(3, 1)
                x, y, z = np.matmul(
                    bunch_of_projections[i], vector) / np.linalg.norm(
    ...
    def plot_rendering(self, ticks):
        ...
        generated by the Grassmannian class but with randomly
        sampled data.
        ...
        if n == 2:
            ...
            self.plot(ticks)
            plot_me = projection_to_two_d(projections,
                                          np.shape(projections)[0])
        ...
        if n == 3:
            ...
            for i in range(points):
                vector = np.random.rand(3, 1)
                x, y, z = np.matmul(bunch_of_projections[i],
    ...
    def parallel_transport(
        ...
        if tangent_vec_b is None:
            if end_point is not None:
                ...
                raise ValueError(
                    "Either an end_point or a tangent_vec_b\
                    must be given to define the"
