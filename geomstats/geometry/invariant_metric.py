...
class _InvariantMetricMatrix(RiemannianMetric):
    ...
    def exp(self, tangent_vec, base_point=None, n_steps=10, step="rk4", **kwargs):
        ...
        def lie_acceleration(state, _time):
            ...
            point, vector = state
            velocity = self._space.tangent_translation_map(point, left=self.left)(
                vector
            )
        ...
        initial_state = gs.stack([base_point, self._space.to_tangent(left_angular_vel)])
...
class _InvariantMetricVector(RiemannianMetric):
    ...
    def exp_from_identity(self, tangent_vec):
        ...
        exp = self._space.regularize(exp)
        return exp
    ...
    def log(self, point, base_point=None, **kwargs):
        ...
        log = self._space.tangent_translation_map(base_point, left=self.left)(
            log_from_id
        )
        return log
...
class BiInvariantMetric(_InvariantMetricVector):
    ...
    def __init__(self, space):
        super().__init__(space=space)
        condition = (
            "SpecialOrthogonal" not in space.__str__()
            and "SO" not in space.__str__()
            and "SpecialOrthogonal3" not in space.__str__()
        )
    ...
    def injectivity_radius(self, base_point):
        ...
        radius : float
            Injectivity radius.
        """
        return gs.pi * self._space.dim**0.5
