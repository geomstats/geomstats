...
class Flag(Manifold):
    ...
    def is_tangent(self, vector, base_point, atol=gs.atol):  # characterization from [Ye2021] Proposition 22
        ...
        for i in range(1, self.d + 1):
            ...
            Z_i = vector[i - 1]
            is_tangent = gs.logical_and(is_tangent, gs.isclose(Matrices.mul(R_i, Z_i) + Matrices.mul(Z_i, R_i), Z_i, atol=atol).all())
            if not gs.any(is_tangent):
                return is_tangent
            is_tangent = gs.logical_and(is_tangent, gs.isclose(Z_i, Matrices.transpose(Z_i), atol=atol).all())
            if not gs.any(is_tangent):
                return is_tangent
            is_tangent = gs.logical_and(is_tangent, gs.isclose(Matrices.mul(R_i, Z_i) + Matrices.mul(Z_i, R_i), Matrices.transpose(Z_i), atol=atol).all())
            if not gs.any(is_tangent):
                return is_tangent
            is_tangent = gs.logical_and(is_tangent, gs.isclose(gs.trace(Z_i), gs.zeros((self.n, self.n)),
                                                               atol=atol).all())
            if not gs.any(is_tangent):
                return is_tangent
            ...
            for j in range(1, i):
                R_j = base_point[j - 1]
                Z_j = vector[j - 1]
                is_tangent = gs.logical_and(is_tangent, gs.isclose(Matrices.mul(Z_i, R_j) + Matrices.mul(R_i, Z_j), gs.zeros((self.n, self.n)),
                                                             atol=atol).all())
                if not gs.any(is_tangent):
                    return is_tangent
