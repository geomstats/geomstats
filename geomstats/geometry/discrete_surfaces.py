...
class DiscreteSurfaces(Manifold):
    ...
    def get_normal(self, point):
        ...
        V0, V1, V2 = point.index_select(0, self.faces[:, 0]), point.index_select(0, self.faces[:, 1]), point.index_select(0, self.faces[:, 2]) 
        N = .5 * gs.cross(V1 - V0, V2 - V0)
        return N        
...
class ElasticMetric(RiemannianMetric):
    ...
    def inner_product(self, tangent_vec_a, tangent_vec_b, base_point):
        ...
        if self.a0>0 or self.a2>0:
            ...
            if self.a2>0:      
                ...
                norm+=self.a2*gs.sum(gs.einsum('bi,bi->b',Del(h),Del(k))/v_areas)
            if self.a0>0:
        ...
        if self.a1>0 or self.b1>0 or self.c1>0 or self.b1>0: 
            ...
            if self.d1>0 or self.b1>0 or self.a1>0:
                ...
                if self.d1>0:
                    xi1=alpha1-alpha
                    xi1_0=gs.matmul(gs.matmul(alpha,ginv),gs.matmul(gs.transpose(xi1,(0,2,1)),alpha)-gs.matmul(gs.transpose(alpha,axes=(1,2)),xi1))
                    xi2=alpha2-alpha
                    xi2_0=gs.matmul(gs.matmul(alpha,ginv),gs.matmul(gs.transpose(xi2,(0,2,1)),alpha)-gs.matmul(gs.transpose(alpha,axes=(1,2)),xi2))   
                    norm+=self.d1*gs.sum(gs.einsum('bii->b',gs.matmul(xi1_0,gs.matmul(ginv,gs.transpose(xi2_0,axes=(0,2,1)))))*areas) 
                if self.b1>0 or self.a1>0:
                    ...
                    norm+=self.a1*gs.sum(gs.einsum('bii->b', gs.matmul(ginvdg1,ginvdg2))*areas)                    
                    norm+=self.b1*gs.sum(gs.einsum('bii->b', ginvdg1)*gs.einsum('bii->b', ginvdg2)*areas)
        return norm      
    ...
    def path_energy(self, path):
        ...
        path_energy : float
            total path energy.
        """
        return .5*gs.sum(self.stepwise_path_energy(path))
    ...
    def _bvp(self, initial_point,end_point):
        ...
        def funopt(midpoint):
            ...
            return  self.path_energy(gs.concatenate([initial_point[None,:,:],midpoint,end_point[None,:,:]],axis=0))
       
        sol = minimize(gs.autodiff.value_and_grad(funopt), gs.flatten(midpoints) , method="L-BFGS-B", jac=True, options={'disp': True,"ftol":.001})
        out=gs.reshape(gs.array(sol.x),(self.time_steps-2,npoints,3))
        geod= gs.concatenate([initial_point[None,:,:],out,end_point[None,:,:]],axis=0)
        return geod        
    ...
    def _stepforward(self, V0,V1):  
        ...
        def funopt(V2):
            ...
            return energy(V2)
        sol = minimize(gs.autodiff.value_and_grad(funopt), gs.flatten(2*(V1-V0)+V0) , method="L-BFGS-B", jac=True,options={'disp': True,"ftol":.00001})
