...
class SpecialLinear(MatrixLieGroup):
    ...
    def projection(self, point):
        ...
        projected : array-like, shape=[..., n, n]
            Projected point.
        """
        proj_point = utils.flip_determinant(gs.copy(point), gs.linalg.det(point))
        scale_coeff = gs.power(gs.linalg.det(proj_point), 1. / self.n)
        return gs.einsum('...ij,...->...ij', proj_point, 1. / scale_coeff)
...
class TracelessMatrices(MatrixLieAlgebra):
    ...
    def _get_basis_nondiag_indices(self):
        return gs.array([i != j for j in range(self.n) for i in range(self.n)])

    def _create_basis(self):
        indices = self._get_basis_nondiag_indices()
        basis = gs.reshape(gs.eye(self.n * self.n)[indices],
                           (self.n**2 - self.n, self.n, self.n))
        diag_basis = []
        diag_rows, diag_cols = gs.diag_indices(self.n)
        for row, col in zip(diag_rows[1:], diag_cols[1:]):
            diag_basis.append(
                gs.array_from_sparse([(0, 0), (row, col)], [-1.0, 1.0], (self.n, self.n)))
    ...
    def basis_representation(self, matrix_representation):
        ...
        """
        indices = gs.reshape(self._get_basis_nondiag_indices(), (self.n, self.n))
    ...
    def projection(self, point):
        ...
        point: array-like, shape=[..., n, n]
            Projected point.
        """
        trace = gs.trace(point, axis1=-2, axis2=-1)
        sps_matrix = gs.array_from_sparse([(0, 0)], [1.0], (self.n, self.n))

        return gs.copy(point) - gs.einsum('...,...ij->...ij', trace, sps_matrix)
    ...
    def random_point(self, n_samples=1, bound=1.0):
        ...
        point : array-like, shape=[..., dim]
           Sample.
        """
        sample = self.ambient_space.random_point(
            n_samples=n_samples, bound=bound)
        return self.projection(sample)
