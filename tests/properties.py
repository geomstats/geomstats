...
class ConnectionParametrizer(Parametrizer):
    def __new__(cls, name, bases, attrs):
        ...
        def test_exp_log_composition(
            self, metric_args, tangent_vec, base_point, rtol, atol
        ):
            metric = self.cls(*metric_args)
            exp = metric.exp(tangent_vec=tangent_vec, base_point=gs.array(base_point))
            result = metric.log(exp, base_point=gs.array(base_point))
