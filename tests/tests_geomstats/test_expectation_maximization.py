...
class TestEM(geomstats.tests.TestCase):
    ...
    def test_fit_init_kmeans(self):
        ...
        means, variances, coefficients = gmm_learning.fit(self.data, lr_mean=5e-3)
    ...
    def test_weighted_frechet_mean(self):
        ...
        expected = self.metric.exp(
            weights[1] / gs.sum(weights) * self.metric.log(data[1], data[0]), data[0]
        )
    ...
    def test_normalization_factor(self):
        ...
        norm_factor_test3, norm_factor_gradient_test = self.metric.norm_factor_gradient(
            variances_test
        )
