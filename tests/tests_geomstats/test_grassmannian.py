...
class TestGrassmannian(geomstats.tests.TestCase):
    ...
    def test_subspace_angle_metric_grassmannian(self):
    ...
    def test_projection_fnorm_metric_grassmannian(self):
            point_a = self.point_a
            point_b = self.point_b
    ...
    def test_projection_2norm_metric_grassmannian(self):
            point_a = self.point_a
            point_b = self.point_b
    ...
    def test_fubini_study_metric_grassmannian(self):
            point_a = self.point_a
            point_b = self.point_b
            
            expected = 1234 #fubini study distance from point a to b
            result = self.metric.fubini_study(point_a, point_b)
            self.assertEqual(result, expected)

    def test_chordal_distance_fnorm_metric_grassmannian(self):
            point_a = self.point_a
            point_b = self.point_b
            
            expected = 1234 #chordal distance fnorm point a to b
            result = self.metric.chordal_distance_fnorm(point_a, point_b)
            self.assertEqual(result, expected)

    def test_chordal_distance_2norm_metric_grassmannian(self):
            point_a = self.point_a
            point_b = self.point_b
