...
def KL_divergence(point, base_point, eps=1e-9):
    ...
    def function_to_integrate(x):
        ...
        safe_denominator = np.where(safe_pdf_base_point_values == 0, eps, safe_pdf_base_point_values)
...
def integrate_func(theta1, theta2):
    # theta1, theta2 = thetas[:2], thetas[2:]
    def _temp_func(x):
        return normal_manifold.point_to_pdf(theta1)(x) * gs.log((normal_manifold.point_to_pdf(theta1)(x))/(normal_manifold.point_to_pdf(theta2)(x)))
    domain = gs.linspace(-10, 10, 1000)
    return trapz(_temp_func(domain), domain)
...
print(stat_metric.metric_matrix(gs.array([1., 5.])))
...
print(fisher_metric.metric_matrix(gs.array([1., 5.])))
