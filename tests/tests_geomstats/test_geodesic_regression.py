...
class TestGeodesicRegression(geomstats.tests.TestCase):
    ...
    def setUp(self):
        ...
        self.shape_se2 = (3, 3)
        #X = gs.random.rand(self.n_samples)
        X = gs.linspace(0., 1., self.n_samples) 
    ...
    def test_loss_hypersphere(self):
        gr = GeodesicRegression(
            self.sphere, metric=self.sphere.metric, center_X=False, method='extrinsic',
        ...
        gr = GeodesicRegression(
            self.sphere, metric=self.sphere.metric, center_X=False, method='riemannian',
            verbose=True, max_iter=50, learning_rate=0.1)
        loss = gr._loss(
            self.X_sphere, 
            self.y_sphere, 
    ...
    def test_fit_riemannian_hypersphere(self):
        ...
        self.assertAllClose(training_score, 1., atol=0.1)
        self.assertAllClose(intercept_hat, self.intercept_sphere_true, atol=5e-3)
    ...
    def test_loss_minimization_extrinsic_se2(self):
        gr = GeodesicRegression(
            self.se2, metric=self.metric_se2, center_X=False, method='extrinsic',
            verbose=True, max_iter=50, learning_rate=0.1)
        ...
        res = minimize(
            objective_with_grad, gs.flatten(self.param_se2_guess), method='CG', jac=True,
            options={'disp': True, 'maxiter': 50})
        ...
        intercept_hat = gs.cast(intercept_hat, dtype=self.y_se2.dtype)
        coef_hat = gs.cast(coef_hat, dtype=self.y_se2.dtype)
    ...
    def test_fit_riemannian_se2(self):
        gr = GeodesicRegression(
            self.se2, metric=self.metric_se2, center_X=False, method='riemannian',
            verbose=True, max_iter=50, learning_rate=0.1)
