...
class TestVisualization(tests.conftest.TestCase):
    ...
    #no upper case    
   ####Adding things #############################################
    def test_plot_Categorical_Distributions_Manifold(self):
        self.CD2.plot()
        self.CD3.plot()
   #delete     
    def test_plot3D_Categorical_Distributions_Manifold(self):
        self.CD2.plot3D()
        self.CD3.plot3D()
    
    def test_scatter_Categorical_Distributions_Manifold(self):
        self.CD2.scatter(self.n_samples)
        self.CD3.scatter(self.n_samples)
    
    def test_set_points_Categorical_Distributions_Manifold(self):
        self.CD2.set_points(self.n_samples)
        
    def test_plot_geodesic_Categorical_Distributions_Manifold(self):
        b2 = np.array([0.2,0.3,0.5])
        v2 = np.array([1,0,0])
        self.CD2.plot_geodesic(v2,b2)
        #we get failure here for b3 and v3
        #b3 = np.array([0.3,0.3,0.2,0.2])
        #v3 = np.array([1,0,0,0])
        #self.CD3.plot_geodesic(v3,b3)
        
    def test_plot_log_Categorical_Distributions_Manifold(self):
        end2 = np.array([0.2,0.3,0.5])
        base2 = np.array([0,0,1])
        self.CD2.plot_log(end2,base2)
        
        end3 = np.array([0.1,0.2,0.3,0.4])
        base3 = np.array([1,0,0,0])
        self.CD3.plot_log(end3,base3)
        
    def test_plot_exp_Categorical_Distributions_Manifold(self):
        b2 = np.array([0.2,0.3,0.5])
        v2 = np.array([1,0,0])
        self.CD2.plot_exp(v2,b2)
        
        b3 = np.array([0.1,0.2,0.3,0.4])
        v3 = np.array([1,0,0,0])
        self.CD3.plot_exp(v3,b3)
############################################################################
